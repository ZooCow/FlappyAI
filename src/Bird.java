...
public class Bird {
    ...
    public void update(int birdX, int birdY, int nearestPipeDistance, int nearestPipeHeight) {
        yvel += gravity;

        if (jumpDelay > 0)
            jumpDelay--;

        if (!dead && (keyboard.isDown(KeyEvent.VK_SPACE) || player.jump(birdX, birdY, nearestPipeDistance, nearestPipeHeight)) && jumpDelay <= 0) {
            yvel = -10;
            jumpDelay = 10;
        }
